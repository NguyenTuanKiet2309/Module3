...
create procedure sp_them_moi_hop_dong(sp_ma_hop_dong int, sp_ngay_lam_hop_dong datetime, sp_ngay_ket_thuc datetime,
                                      sp_tien_dat_coc double, sp_ma_nhan_vien int, sp_ma_khach_hang int,
                                      sp_ma_dich_vu int)
begin
	if
sp_ma_hop_dong in (select ma_hop_dong from hop_dong) then
		SIGNAL SQLSTATE '50000'
		SET MESSAGE_TEXT = 'Mã hợp đồng đã tồn tại';
end if;
	if
sp_ma_nhan_vien not in (select ma_nhan_vien from hop_dong) then
		SIGNAL SQLSTATE '55000'
		SET MESSAGE_TEXT = 'Mã nhân viên không tồn tại';
end if;
	if
sp_ma_dich_vu not in (select ma_dich_vu from hop_dong) then
		SIGNAL SQLSTATE '60000'
		SET MESSAGE_TEXT = 'Mã dịch vụ không tồn tại';
end if;
	if
sp_ma_khach_hang not in (select ma_khach_hang from hop_dong) then
...
insert into hop_dong(ma_hop_dong, ngay_lam_hop_dong, ngay_ket_thuc, tien_dat_coc, ma_nhan_vien, ma_khach_hang,
                     ma_dich_vu)
values (sp_ma_hop_dong, sp_ngay_lam_hop_dong, sp_ngay_ket_thuc, sp_tien_dat_coc, sp_ma_nhan_vien, sp_ma_khach_hang,
        sp_ma_dich_vu);
end
//
delimiter ;
